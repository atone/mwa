...
public class NABRNNAlignmentTrainer implements AlignmentTrainer<NABRNNAligner> {

	private DataSet dataset;
	private BRNNSimAligner aligner;

	public NABRNNAlignmentTrainer(Aligner aligner) {
		this.aligner = (BRNNSimAligner) aligner;
	}

	@Override
	public NABRNNAligner train(List<TrainingPair> data) {
		...
		NABRNNAligner naBrnnSimAligner = new NABRNNAligner(naNN, aligner);
		return naBrnnSimAligner;
	}

	@Override
	public NABRNNAligner load(File file) throws IOException {
		NABRNNAligner naBrnnSimAligner = SerializationUtils.readObject(file);
		return naBrnnSimAligner;
	}
